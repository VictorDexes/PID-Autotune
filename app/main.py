...
class MspClient:
    def __init__(self, port: str, baudrate: int = 115200) -> None:
        if serial is None:
            raise MspError("pyserial n'est pas installe.")
    ...
    def command(self, command_id: int, payload: bytes = b"") -> bytes:
        if len(payload) > 255:
            raise MspError("Payload MSP v1 trop grand.")
    ...
    def _read_response(self, expected_command_id: int) -> bytes:
        ...
        if header != b"$M>":
            if header == b"$M!":
                raise MspError(f"Betaflight a refuse la commande {expected_command_id}.")
            raise MspError(f"Reponse MSP invalide: {header!r}")
        ...
        if checksum != received_checksum:
            raise MspError("Checksum MSP invalide.")
        if command_id != expected_command_id:
            raise MspError(
                f"Commande inattendue: {command_id}, attendu {expected_command_id}."
            )
    ...
    def _read_exact(self, size: int) -> bytes:
        ...
        if len(data) != size:
            raise MspError("Timeout pendant la lecture MSP.")
        return data
...
def parse_dataflash_summary(payload: bytes) -> DataflashSummary:
    if len(payload) < 13:
        raise MspError("Resume Dataflash trop court.")
...
def parse_dataflash_read(payload: bytes, expected_address: int) -> bytes:
    ...
    if address != expected_address:
        raise MspError(
            f"Adresse Dataflash inattendue: {address}, attendu {expected_address}."
        )
    return payload[4:]
